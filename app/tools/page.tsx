      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {tools.map((tool) => {
              const isLinked = tool.status === "new" || tool.status === "available";
              const cardContent = (
                <>
                  <div className="flex items-start justify-between mb-5">
                    ...
                    {tool.status === "new" && (
                      <span className="text-xs font-semibold text-white px-2.5 py-1 rounded-full" style={{ backgroundColor: "#FC4C02" }}>
                        New
                      </span>
                    )}
                  </div>
              ...
              return isLinked ? (
                <Link
                  ...
                  className="block bg-white border border-zinc-100 rounded-2xl p-8 shadow-sm hover:shadow-md hover:border-zinc-200 transition-all"
                >
                  {cardContent}
                </Link>
              ) : (
